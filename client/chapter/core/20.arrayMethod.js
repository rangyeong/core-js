...
const rainbow = arr1.concat(arr2)
console.log(rainbow)
...
// 제너레이터 펑션 - 이터레이터 객체를 아주 쉽게 만들 수 있음. arrow function은 불가능. 
function* gen(){
  yield 1;
  yield 2;
  yield 3;
...
const gene = gen(); // 이렇게 담아서 사용해야지 값이 기억됨
...
for(const a of customIter){
  console.log(a) // 객체를 순환할 수 console.error('있음',있음)
}

function* idGenerator(){
  // let id = 1;
  while(true){
    yield `user=${crypto.randomUUID()}` // 랜덤 ID가 무한으로 생성됨.
  }
}
